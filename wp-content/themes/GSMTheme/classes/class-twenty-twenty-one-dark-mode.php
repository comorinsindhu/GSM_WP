...
class Gsm_Theme_Dark_Mode {
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && Gsm_Theme_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
	...
	public function customizer_controls( $wp_customize ) {
		...
		$wp_customize->add_control(
			new Gsm_Theme_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => static function() {
						return 127 >= Gsm_Theme_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: Gsm Theme support article URL. */
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= Gsm_Theme_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= Gsm_Theme_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Gsm Theme uses LocalStorage when Dark Mode support is enabled.', 'twentytwentyone' ) . '</p>'
		...
		wp_add_privacy_policy_content( __( 'Gsm Theme', 'twentytwentyone' ), wp_kses_post( wpautop( $content, false ) ) );
	}
